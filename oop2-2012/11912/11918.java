...
public class Register {
	...
	public static void regPryl() {
		try {
			...
			if (m == null)
				...
				System.out.println("Hur manga aktier har du?");
				...
				System.out.println();
				System.out
					.println("Vad ar det for typ av smycke?");
				String smyckeTyp = keyboard.nextLine();
				System.out
					.println("ar smycket av guld eller silver?");
				String guldSilver = keyboard.nextLine();
				System.out
					.println("Hur manga adelstenar har smycket?");
				...
				System.out
					.println("Vad ar priset pa apparaten?");
				int nPris = Integer.parseInt(keyboard
					.nextLine());
				System.out
					.println("Vad ar slitskadorna? 1-10, 10 betyder helt ny och 1 mycket sliten");
	...
	public static void listaRikaste() {
		...
		for (int i = 0; i < allaPersoner.size(); i++){
			...
			//----------------	
				System.out.println("Rikaste ar " + allaPersoner.get(pIndex).getNamn() + " som sammanlagt ager " + allaPersoner.get(pIndex).getVarde() + "\n");	
	...
	public static void listaPerson() {
		System.out.println("Uppge namn pa den du soker:");
		...
		if (namn){
			System.out.println(allaPersoner.get(indexPerson).toString()) ;
			allaPersoner.get(indexPerson).listaPrylar();
		}
